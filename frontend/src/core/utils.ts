...
import { circuitEpochTreeDepth, circuitGlobalStateTreeDepth, circuitNullifierTreeDepth, circuitUserStateTreeDepth, epochLength, epochTreeDepth, globalStateTreeDepth, maxUsers, nullifierTreeDepth, numAttestationsPerEpochKey, numEpochKeyNoncePerEpoch, userStateTreeDepth } from '../config/testLocal'
...
const genUnirepStateFromContract = async (
    ...
    for (let i = 0; i < sequencerEvents.length; i++) {
        ...
        if (occurredEvent === "UserSignUp") {
            ...
            epochKeys_ = epochKeys_.map((epk: any) => BigInt(epk.toString()))
            epochKeyHashchains_ = epochKeyHashchains_.map((hc: any) => BigInt(hc.toString()))
            ...
            const attestationNullifiersInEvent = userStateTransitionedEvent.args?.userTransitionedData.attestationNullifiers.map((nullifier: any) => BigInt(nullifier) % BigInt(2 ** unirepState.nullifierTreeDepth))
            const epkNullifiersInEvent = userStateTransitionedEvent.args?.userTransitionedData.epkNullifiers.map((nullifier: any) => BigInt(nullifier) % BigInt(2 ** unirepState.nullifierTreeDepth))
            // Combine nullifiers and mod them
            const allNullifiersInEvent = attestationNullifiersInEvent.concat(epkNullifiersInEvent).map((nullifier: any) => BigInt(nullifier) % BigInt(2 ** unirepState.nullifierTreeDepth))
